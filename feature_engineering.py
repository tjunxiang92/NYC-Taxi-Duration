...
def process(fname, isTrainFile = False):
    df = pd.read_csv(fname)

    # Data Cleaning only applies to training file
    if isTrainFile:
    ...
    # Calculate straight line distance between pickup & dropoff in km
    def get_distance(x):
        coords_1 = (x[0], x[1])
        coords_2 = (x[2], x[3])
    ...
    # This function queries the OSRM application to find a route from the pickup to dropoff location. It provides a distance parameter for the shortest path traveled by car.
    ...
    df.to_csv('processed_{}'.format(fname), index=False)

# Run the script on the train & test files provided by the Kaggle Dataset
def main():
    process("train.csv", True)
    process("test.csv")
